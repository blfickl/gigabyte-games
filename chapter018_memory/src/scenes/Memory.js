// Memory.js - Main scene for digestion memory game
class Memory extends Phaser.Scene {
    ...
    create() {
        ...
        // Layout grid: 4 rows, 3 columns (for 12 cards)
        this.cardWidth = 110;
        this.cardHeight = 70;
        this.cardMargin = 12;
        this.cols = 3;
    ...
    flipCard(card) {
        ...
        if (!card.textObj) {
            card.textObj = this.add.text(card.x, card.y, card.cardName, { font: '16px Arial', color: '#fff', wordWrap: { width: this.cardWidth - 10 } }).setOrigin(0.5);
        } else {
    ...
    showCongratsPopup() {
        ...
        const bullets = [
            'Hydration: Water is essential for every step of digestion and nutrient absorption.',
            'Fermentation: Microbes in the gut break down fiber, producing energy and nutrients.',
            'Saliva: Moistens feed and begins the digestive process in the mouth.',
            'Esophagus: The muscular tube that moves food from the mouth to the stomach.',
            'Colon: The site of water absorption and final fermentation before waste is expelled.',
            'Stomach acid: Breaks down food in the stomach and helps kill harmful microbes.'
        ];
        let bulletY = popupY - 80;
        for (let i = 0; i < bullets.length; i++) {
            this.add.text(popupX - popupWidth / 2 + 40, bulletY + i * 32, '• ' + bullets[i], { font: '15px Arial', color: '#222', wordWrap: { width: popupWidth - 80 } }).setOrigin(0, 0.5);
        }
        // Close button
